    ...
    <div class="container-fluid">
        <div class="block-header">
            <h2>
                ...
                <a href="{{url('app/kreiraj/baner')}}" class="btn btn-success btn-circle waves-effect waves-circle waves-float">
        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    ...
                    <div class="body">
                        <form action="{{url('app/store/baner/veliki')}}" method="POST" >
                            {!! csrf_field() !!}
                            <select id="optgroup" class="ms" multiple="multiple" name="slajder[]">
                                @foreach($baneri as $smestaj)
                                    @if($smestaj->veliki == true)
                                        <option value="{{$smestaj->id}}" selected>{{$smestaj->url}}</option>
                                    @else
                                        <option value="{{$smestaj->id}}" >{{$smestaj->url}}</option>
                                    @endif
                                @endforeach
                            </select>
        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    ...
                    <div class="body">
                        <form action="{{url('app/store/baner/mali')}}" method="POST" >
                            {!! csrf_field() !!}
                            <select id="optgroup2" class="ms" multiple="multiple" name="slajder[]">
                                @foreach($baneri as $smestaj)
                                    @if($smestaj->mali == true)
                                        <option value="{{$smestaj->id}}" selected>{{$smestaj->url}}</option>
                                    @else
                                        <option value="{{$smestaj->id}}" >{{$smestaj->url}}</option>
                                    @endif
        ...
        <div class="row clearfix">
            ...
            <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                <div class="card">
                    <div class="body">
                        <img class="img-responsive" src="{{url($smestaj->slika)}}">
                    </div>
                        ...
                        <ul class="header-dropdown m-r--5">
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu pull-right">
                                    <li><a href="{{url('app/edit/baner/'.$smestaj->id)}}">Edit</a></li>
                                    <li><a href="{{url('app/delete/baner/'.$smestaj->id)}}">Izbrisi</a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
